...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* ⛔ Every surface colour here MUST be a theme token. A literal light value stays light in
     dark mode while its text follows --navy, which is #1a365d in light and #ffffff in dark,
     so a hardcoded white background renders white-on-white and the control disappears. */
  .seg button{appearance:none;border:0;background:var(--bg-card);color:var(--navy);
  ...
  select{padding:8px 10px;font-size:14px;font-family:inherit;
    border:1px solid var(--line);border-radius:6px;background:var(--bg-card);color:var(--body);min-width:260px}

  .card{margin-top:16px;border:1px solid var(--line);border-radius:8px;overflow:hidden}
  .card .head{padding:14px 18px;background:var(--alt);border-bottom:1px solid var(--line)}
...
$('#stamp').innerHTML = 'Series concept DOI (always resolves to the latest issue): '
  ...
  /* ⛔ The apostrophe in "author&#39;s" MUST stay an HTML entity. Written literally it closes
     this single-quoted JS string, the whole script block fails to parse, and the page renders
     as an empty shell: no subtitle, no signal text, no edition buttons, no cards. */
